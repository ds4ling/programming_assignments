...
```{r}
#| label: load-data
dat <- read_csv(here("ratings_data_2025-04-23.csv"))
```
...
> During 2023, the average enjoyment was `r round(coef(mod_q1_1)[1], 2)` $\pm$ 0.05 standard errors (t = 11.83, p < 0.001), indicating that average enjoyment was not statistical equivalent to 0. 
> For 2025, the average enjoyment increased realtive to 2023 by `r round(coef(mod_q1_1)[2], 2)` $\pm$ 0.09 standard errors (t = 0.77, p = 0.45), but this difference is not statistically significant. 
...
This implies that, not only is the DV is not normally distributed, it also does not range from -∞ to ∞. 
